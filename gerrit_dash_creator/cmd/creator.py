...
def load_template(template_file=None, template_directory=None,
    ...
    if template_file:
        template_directory = os.path.basename(template_file)
        template_name = os.path.dirname(os.path.abspath(template_file))
...
def generate_dashboard_urls(paths, template):
    result = 0

    for dashboard in load_dashboards(paths):
        try:
...
def main():
    ...
    try:
        if template and opts.check_only:
            load_dashboards(opts.dashboard_paths)
        elif template:
            generate_dashboard_urls(opts.dashboard_paths, template)
        else:
            return 1
    except ValueError as e:
        print("error: %s" % e)
